...
func NewEmulatorAPIServer(emulator emulator.Emulator, adapter *adapters.AccessAdapter) *EmulatorAPIServer {
	...
	router.HandleFunc("/emulator/profiling", r.Profiling).Methods("GET")
...
func (m EmulatorAPIServer) Profiling(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")

	err := json.NewEncoder(w).Encode(m.emulator.ProfilingReport())
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
